...
final_logit_lasso <- finalize_workflow(rec_wf, best_acc)

print("*********** LOGISTIC REGRESSION **************")
logit_test <- last_fit(final_logit_lasso, income_split) %>%
  collect_metrics()

logit_test %>% print(n = 3)
top_acc %>% print(n = 5)
...
nnet_parm_df <- nnet_parm_df1 %>%
  cross_join(lambda_grid)
...
# define a smaller, more stable grid
svm_parm_df1 <- tibble(cost = c(2^(-2), 2^(-1), 2^0, 2^1, 2^2))
svm_parm_df2 <- tibble(rbf_sigma = c(2^(-2), 2^(-1), 2^0, 2^1, 2^2))
svm_parm_df  <- svm_parm_df1 %>%
  cross_join(svm_parm_df2)
